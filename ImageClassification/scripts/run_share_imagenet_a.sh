#!/usr/bin/env bash
# =============================================================================
# run_share_imagenet_a.sh — Share continual learning on ImageNet-A (200 classes)
# =============================================================================
#
# SD-Lora-CL settings: 20-class increments (10 tasks), rank=10, lr=5e-3,
# 20 epochs per task, milestones=[10,15], lrate_decay=0.05, weight_decay=5e-4.
#
# Data layout expected on disk:
#   <DATA_ROOT>/imagenet-a/train/<class_folder>/
#   <DATA_ROOT>/imagenet-a/test/<class_folder>/
#
# Usage:
#   ./scripts/run_share_imagenet_a.sh [BASE_OUTPUT_DIR] [DATA_ROOT]
# =============================================================================

set -euo pipefail

BASE_OUTPUT_DIR="${1:-./share_imagenet_a_outputs}"
DATA_ROOT="${2:-./data}"
...
USE_WANDB="${USE_WANDB:-}"
WANDB_PROJECT="${WANDB_PROJECT:-ViT_Share_ImageNetA}"
...
phase1_lora_bootstrap() {
    log "Phase 1: LoRA Bootstrap — ImageNet-A (all ${SUBSET_SIZE}-class subsets)"
    python train_vit.py \
        --method lora \
...
train_eigenflux_subset() {
    ...
    log "Phase 3 [T-${idx}]: Training subset ${subset_num} — ImageNet-A"
    python train_vit.py \
        --method eigenflux \
        --model_name "${MODEL_NAME}" \
        --eigenflux_r "${EIGENFLUX_R}" \
...
phase3_continual_training() {
    log "Phase 3: Continual EigenFlux Training — ImageNet-A"
    local num_subsets
...
main() {
    log "Share ImageNet-A Continual Learning Pipeline"
